...
Módulo do **GestIQ** responsável pelo gerenciamento de acesso Wi-Fi para hóspedes e visitantes utilizando portal cativo integrado ao MikroTik.
...
[https://wifi.hotel/acesso/7H3K92](https://wifi.hotel/acesso/7H3K92)
Validade: 4 horas
...
O módulo utiliza inicialmente tabelas principais para controle de visitantes, tokens, sessões, configurações e múltiplos estabelecimentos.

## wifi_visitantes

Armazena os visitantes identificados pelo sistema.
...
* manter histórico por número
* identificar recorrência
* permitir bloqueio ou análise futura

## wifi_tokens

Controla os tokens de acesso enviados para validação.
...
* validar clique no link
* controlar expiração
* evitar reutilização indevida

## wifi_sessoes

Registra as sessões reais de uso da rede.
...
* registrar uso real da rede
* permitir auditoria
* gerar métricas e relatórios

## wifi_config

Guarda parâmetros configuráveis do módulo por unidade.

Campos principais:

* id
* estabelecimento_id
* tempo_visitante_horas
* max_dispositivos_visitante
* velocidade_download_kbps
* velocidade_upload_kbps
* max_acessos_por_dia
* exigir_aceite_termos
* ativo

Objetivo:

* permitir configuração por hotel/local
* evitar regras fixas no código

## wifi_estabelecimentos

Tabela estratégica para o futuro SaaS do módulo.
...
* separar dados por cliente/unidade
* suportar vários hotéis, pousadas ou empresas
* preparar o módulo para comercialização

> Observação: esta tabela quase não faz falta no começo para uso interno em um único hotel, mas é muito importante se o projeto evoluir para produto multiempresa.

## Scripts SQL

Os scripts de criação e manutenção do banco devem ficar em:
