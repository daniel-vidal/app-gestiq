...
function parseDataIsoParaUtcMs(dataIso) {
...
function extrairDataIsoCurta(valor) {
...
function normalizarData(data) {
  ...
  const isoCurta = extrairDataIsoCurta(data);
  if (isoCurta) return isoCurta;
...
function calcularPeriodoEstadia(checkin, checkout) {
  ...
  const dtCheckinUtc = parseDataIsoParaUtcMs(checkin);
  const dtCheckoutUtc = parseDataIsoParaUtcMs(checkout);

  if (dtCheckinUtc == null || dtCheckoutUtc == null) return null;

  const diffMs = dtCheckoutUtc - dtCheckinUtc;
...
function calcularAntecedenciaCompra(dataConsultaIso, checkin) {
  ...
  const dataConsultaCurta = normalizarData(dataConsultaIso);
  const checkinCurto = normalizarData(checkin);

  const dtConsultaUtc = parseDataIsoParaUtcMs(dataConsultaCurta);
  const dtCheckinUtc = parseDataIsoParaUtcMs(checkinCurto);

  if (dtConsultaUtc == null || dtCheckinUtc == null) return null;

  const diffMs = dtCheckinUtc - dtConsultaUtc;
