  ...
  try {
    for (const tarifa of tarifasNormalizadas) {
      ...
      if (!atual) {
        ...
        await inserirTarifaAtual(client, tarifa, historico);
        resumo.novas += 1;
        continue;
      }
      ...
      if (mesmaTarifa(atual, tarifa)) {
        ...
        await atualizarTarifaAtual(client, atual.id, tarifa, historicoAtualizado);
        resumo.iguais += 1;
        continue;
      }

      const novoHistorico = await inserirHistorico(client, tarifa);
      await atualizarTarifaAtual(client, atual.id, tarifa, novoHistorico);
      resumo.alteradas += 1;
    }
